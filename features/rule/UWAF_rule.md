...
带 * 的匹配字段标识其为键值结构，这些字段的匹配内容可以接收一个 JSON 字符串以针对单个键的值、键名、值名或全部键、值名进行匹配。**若输入普通字符串或不符合JSON格式的字符串，则默认对全部键名、值进行匹配**。若键值结构匹配字段对应的操作符为长度大于/小于，在没有指定**特定键的值**的情况下则比较的是键/值总个数。
